...
def parse(app, name):
    ...
    try:
        ...
        start_c = result[0]
        start_idx = result[1]
        rows = st.range('%s%s' % ('I', start_idx+1)).expand('down')
        row_count = len(rows)
...
def get_titles(st, start_c, start_idx):
    ...
    if len(titles) > 0:
        for t in range(0, len(titles)):
            ...
            elif u'影院编号' == title or u'影院ID' == title or u'编号'== title:
            ...
            elif u'专资编码' == title or u'专资代码' == title or u'影院代码' == title:
...
def read_rows_data(st, titles, start_c, start, count):
    if os.path.exists(os.getcwd()+'\\wanda.pk'):
        with open(os.getcwd()+'\\wanda.pk', 'rb') as f:
        ...
        for i in range(0, count):
            ...
            cinema.providor = Providor.WANDA
        ...
        print '\n'
        with open(os.getcwd()+'\\wanda.pk', 'wb') as f:
